...
export default function InvestSandbox() {
  ...
  const [portfolio, setPortfolio] = useState<Portfolio>({
    ...
    holdings: {
      ...
      // TSLA: { quantity: 5, purchasePrice: 200.0, currentPrice: 180.50 }, // Example, keeping only AAPL initially
    },
  });

  const { totalStockValue, totalValue } = React.useMemo(() => {
    let currentStockValue = 0;
    for (const symbol in portfolio.holdings) {
      const holding = portfolio.holdings[symbol];
      const currentPriceFromDb = mockStockDatabase[symbol]?.price || holding.currentPrice;
      currentStockValue += currentPriceFromDb * holding.quantity;
    }
    const calculatedTotalValue = portfolio.cash + currentStockValue;
    return { totalStockValue: currentStockValue, totalValue: calculatedTotalValue };
  }, [portfolio]);
  ...
  const handleTrade = (type: 'buy' | 'sell') => {
    ...
    const tradeValue = currentPrice * numQuantity;
    ...
    if (type === 'buy') {
      ...
      setPortfolio(prevPortfolio => {
        ...
        const existingHolding = newHoldings[symbol];

        if (existingHolding) {
          const existingTotalValue = existingHolding.purchasePrice * existingHolding.quantity;
          const newTotalQuantity = existingHolding.quantity + numQuantity;
          const newAveragePrice = (existingTotalValue + tradeValue) / newTotalQuantity;

          newHoldings[symbol] = {
            ...existingHolding,
            quantity: newTotalQuantity,
            purchasePrice: newAveragePrice,
            currentPrice: mockStockDatabase[symbol]?.price || currentPrice,
          };
        } else {
          newHoldings[symbol] = {
      ...
      if (!existingHolding || existingHolding.quantity < numQuantity) {
        alert(`You only own ${existingHolding?.quantity || 0} shares of ${symbol}.`);
        return;
      }

      setPortfolio(prevPortfolio => {
        const newHoldings = { ...prevPortfolio.holdings };
        newHoldings[symbol].quantity -= numQuantity;
    ...
    // Alert and clear quantity remains the same
  ...
  return (
    <div className="p-5 pb-20"> {/* Added bottom padding */}
      ...
      {/* --- Your Holdings --- */}
      <div className="bg-white/60 backdrop-blur-md rounded-lg shadow-lg p-6 mb-6 max-h-[50vh] overflow-y-auto">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4 sticky top-0 bg-white/60 backdrop-blur-md py-2 -mt-6 -mx-6 px-6 z-10">Your Holdings</h2>
        <div className="space-y-4 pt-2">
          ...
          {Object.keys(portfolio.holdings).map((stockSymbol) => {
            ...
            const currentPriceFromDb = mockStockDatabase[stockSymbol]?.price || holding.currentPrice;
            const currentValue = currentPriceFromDb * holding.quantity;
            const profitLoss = (currentPriceFromDb - holding.purchasePrice) * holding.quantity;
            ...
            return (
              <div className="p-4 bg-white/70 rounded-lg" key={stockSymbol}>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-gray-500">{holding.quantity} Shares @ avg ${holding.purchasePrice.toFixed(2)}</span>
