...
// --- MOCK DATABASE (with multiple stocks) ---
const mockStockDatabase: { [key: string]: StockInfo } = {
  MSFT: {
    ...
    changePercent: 30.61,
    chartData: [
      { name: 'May', price: 195.8 }, { name: 'Jun', price: 202.1 }, { name: 'Jul', price: 200.4 },
      { name: 'Aug', price: 228.7 }, { name: 'Sep', price: 235.1 }, { name: 'Oct', price: 252.29 },
    ],
  ...
  AAPL: {
    ...
    changePercent: 6.05,
    chartData: [
      { name: 'May', price: 165.2 }, { name: 'Jun', price: 168.0 }, { name: 'Jul', price: 170.1 },
      { name: 'Aug', price: 172.5 }, { name: 'Sep', price: 173.0 }, { name: 'Oct', price: 175.2 },
    ],
  ...
  GOOG: {
    ...
    changePercent: 12.38,
    chartData: [
      { name: 'May', price: 124.1 }, { name: 'Jun', price: 122.5 }, { name: 'Jul', price: 128.9 },
      { name: 'Aug', price: 135.2 }, { name: 'Sep', price: 132.1 }, { name: 'Oct', price: 139.74 },
    ],
  ...
  TSLA: {
    ...
    changePercent: -9.93,
    chartData: [
      { name: 'May', price: 190.4 }, { name: 'Jun', price: 188.1 }, { name: 'Jul', price: 185.0 },
      { name: 'Aug', price: 182.5 }, { name: 'Sep', price: 181.0 }, { name: 'Oct', price: 180.5 },
    ],
...
const InvestSandbox = () => {
  ...
  const [portfolio, setPortfolio] = useState<Portfolio>({
    ...
    holdings: {
      ...
      TSLA: { quantity: 5, purchasePrice: 200.0, currentPrice: 180.50 },
    },
  });
  ...
  const handleSearch = () => {
    ...
    if (data) {
      ...
      alert(`Symbol "${searchSymbol}" not found in simulator. Try AAPL, MSFT, GOOG, or TSLA.`);
      setStockInfo(null);
    }
    setQuantity('');
  };
  ...
  const handleTrade = (type: 'buy' | 'sell') => {
    ...
    if (!stockInfo) {
      alert('Error: Please search for a stock first.');
      return;
    }
    ...
    alert(`Simulating ${type} of ${numQuantity} ${stockInfo.symbol}`);
    setQuantity('');
  };
  ...
  return (
    <div className="p-5 max-w-md mx-auto">
      ...
      <p className="text-xs text-gray-500 text-center mb-5 px-4">
        This is a simulator. All prices are for educational purposes and do not
        involve real money.
      </p>
                ...
                <div className="h-24 w-full mb-3">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                      ...
                      data={mockStockDatabase[stockSymbol]?.chartData}
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button
            ...
            className="bg-gray-800 text-white p-3 rounded-md font-semibold hover:bg-gray-700 transition-colors"
          >
            Search
          </button>
        </div>

        {/* --- Conditional UI - Shows After Search --- */}
        {stockInfo && (
          <div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Line
                    type="monotone"
                    dataKey="price"
                    stroke={stockInfo.change >= 0 ? '#22c55e' : '#ef4444'} // Chart color matches profit/loss
                    strokeWidth={3}
                    dot={false}
                    activeDot={{ r: 6 }}
                  />
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-4">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-gray-800">{stockInfo.open}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Mkt Cap</span>
                <span className="font-medium text-gray-800">{stockInfo.mktCap}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">High</span>
                <span className="font-medium text-gray-800">{stockInfo.high}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">P/E Ratio</span>
                <span className="font-medium text-gray-800">{stockInfo.peRatio}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Low</span>
                <span className="font-medium text-gray-800">{stockInfo.low}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Div Yield</span>
                <span className="font-medium text-gray-800">{stockInfo.divYield}</span>
              </div>
            </div>

            {/* --- Trade Inputs (now appear after search) --- */}
            <input
              type="number"
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 bg-blue-600 text-white p-3 rounded-md font-semibold hover:bg-blue-700 transition-colors"
              >
                Buy
              </button>
              <button
